...
export async function GET(
  ...
  try {
    ...
    const applydata = await db.submittions.findFirst({
      where: {
        userId: userdata.id,
        postId: hackId,
        stageId: params.stageId,
    ...
    if (!applydata) {
      return new NextResponse("No application found for this stage", {
        status: 404,
      });
    }
...
export async function DELETE(
  ...
  try {
    ...
    const applydata = await db.submittions.deleteMany({
      where: {
        userId: userdata.id,
        postId: hackId,
        stageId: params.stageId,
